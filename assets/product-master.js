...
async function productMasterPage(){
  ...
  async function createShopifyDraft(r){
    ...
    try{
      const collection=shopifyCollection(r.collection),result=await cloudFetch('/shopify/products/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({product:{title:r.name,descriptionHtml:r.full_description||r.short_description||'',vendor:'PLA Pals',productType:'PLA Pal',tags:['PLA Pals',collection].filter(Boolean),collection,status:r.on_sale?'ACTIVE':'DRAFT',price:r.price,sku:r.sku,barcode:r.barcode||r.sku},image:{}})});
  ...
  async function uploadShopifyImage(r){
    ...
    try{
      ...
      const collection=shopifyCollection(r.collection),result=await cloudFetch('/shopify/products/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({product:{title:r.name,descriptionHtml:r.full_description||r.short_description||'',vendor:'PLA Pals',productType:'PLA Pal',tags:['PLA Pals',collection].filter(Boolean),collection,status:r.on_sale?'ACTIVE':'DRAFT',price:r.price,sku:r.sku,barcode:r.barcode||r.sku},image:{filename:file.name,content_type:file.type,content_base64:encoded}})});

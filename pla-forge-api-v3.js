...
const ROLE_PERMISSIONS = {
  admin: ["*"],
  factory: [
    "core:read","products:read","recipes:read","filaments:read","filaments:write",
    "targets:read","targets:write","inventory:read","consumables:read",
    "production:read","production:write","dispatch:read","dispatch:write"
  ],
  cornwall: [
    "core:read","products:read","recipes:read","filaments:read","targets:read",
    "inventory:read","consumables:read","dispatch:read","cornwall:read","cornwall:write",
    "rework:read","rework:write"
  ],
  viewer: [
    "core:read","products:read","recipes:read","filaments:read","targets:read",
    "inventory:read","consumables:read","dispatch:read","rework:read"
  ]
};

function corsHeaders(request) {
  const origin = request.headers.get("Origin");
  const allowed = origin && ALLOWED_ORIGINS.has(origin) ? origin : "https://plapalsuk.github.io";
  return {
...
async function passwordHash(password,salt){
  ...
  const bits=await crypto.subtle.deriveBits(
    {name:"PBKDF2",hash:"SHA-256",salt:textBytes(salt),iterations:120000},key,256
  );
  return b64url(bits);
}
...
export default {
  async fetch(request,env){
    ...
    try{
      if(request.method==="GET"&&path==="/")return json(request,{success:true,service:"PLA Forge API",auth:"employee"});
